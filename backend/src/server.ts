...
const permissionsForUser = async (userId: unknown) => {
  if (typeof userId !== "string" || !userId) {
    return { role: "admin", permissions: [...rolePermissions.admin], organizationId: null };
  }
  const { rows } = await pool.query(
    `select organization_id, role from org_members where user_id=$1
     order by case role when 'admin' then 0 when 'model_maintainer' then 1 else 2 end limit 1`,
    [userId],
  );
  const role = (rows[0]?.role ?? "viewer") as keyof typeof rolePermissions;
  return { role, permissions: [...rolePermissions[role]], organizationId: rows[0]?.organization_id ?? null };
};

const camelCase = (key: string) => key.replace(/_([a-z])/g, (_match, letter: string) => letter.toUpperCase());
...
const requireProjectUploadAccess = async (projectId: string, userId: unknown) => {
  if (typeof userId !== "string" || !userId) {
...
const requireModelMaintainerAccess = async (userId: unknown) => {
  if (!isSupabaseAuthConfigured() && !process.env.JWT_SECRET) return;
  if (typeof userId !== "string" || !userId) {
...
app.use(async (req, res, next) => {
  ...
  if (!supabaseConfigured && !secret) return next();
...
app.get("/api/dashboard", async (_req, res) => {
  ...
  const [projects, materials, tests, reports, models] = await Promise.all([
    ...
    pool.query(
      `select count(*)::int as count from model_registry${organizationId ? " where organization_id=$1 and active=true" : " where active=true"}`,
      values,
    ),
  ]);
  const recentTests = await pool.query(
    `select id,test_type,status,created_at from material_tests${orgFilter} order by created_at desc limit 5`,
    values,
  );
  res.json({
    role: access.role,
...
app.post(["/projects", "/api/projects"], async (req, res) => {
  ...
  const { rows } = await pool.query(
    `insert into projects(name,description,location,client_name,status)
     values($1,$2,$3,$4,$5) returning *`,
    [body.name, body.description, body.location, body.clientName, body.status],
  );
...
app.patch(["/projects/:projectId", "/api/projects/:projectId"], async (req, res) => {
  ...
  const { rows } = await pool.query(
    ...
    [projectId, body.name, "description" in body, body.description,
...
app.get("/projects/:projectId/concrete-mixes", async (req, res) => {
  const { rows } = await pool.query(
    "select * from concrete_mixes where project_id=$1 order by created_at desc",
...
app.post("/projects/:projectId/concrete-mixes", async (req, res) => {
  ...
  const { rows } = await pool.query(
    ...
    [req.params.projectId, body.name, body.binderType, body.cementKg, body.flyAshKg, body.ggbssKg,
...
app.post("/api/projects/:projectId/materials", async (req, res) => {
  ...
  const { rows } = await pool.query(
    `insert into materials(project_id,name,category,spec,supplier,unit_cost,co2_factor)
     values($1,$2,$3,$4,$5,$6,$7) returning *`,
    [projectId, body.name, body.category, JSON.stringify(body.spec), body.supplier, body.unitCost, body.co2Factor],
  );
  res.status(201).json(toCamelCase(rows[0]));
});
...
app.get("/api/tests", async (req, res) => {
  const query = z.object({ projectId: z.string().uuid() }).parse(req.query);
  const { rows } = await pool.query(
    "select * from material_tests where project_id=$1 order by created_at desc",
    [query.projectId],
  );
  res.json({ items: toCamelCase(rows) });
});
...
app.post("/api/tests", async (req, res) => {
  ...
  const { rows } = await pool.query(
    ...
    [body.projectId, body.materialId, body.bimElementId, body.testType, body.ageDays,
...
app.post("/api/projects/:projectId/tests", async (req, res) => {
  ...
  const { rows } = await pool.query(
    ...
    [projectId, body.materialId, body.bimElementId, body.testType, body.ageDays,
      JSON.stringify(body.inputFeatures), JSON.stringify(body.measuredValues),
      JSON.stringify(body.predictedValues), body.modelName, body.confidence, body.status],
  );
  res.status(201).json(toCamelCase(rows[0]));
...
app.get("/api/bim/elements", async (req, res) => {
  const query = z.object({ projectId: z.string().uuid() }).parse(req.query);
  const { rows } = await pool.query(
    "select * from bim_elements where project_id=$1 order by created_at desc",
    [query.projectId],
  );
  res.json({ items: toCamelCase(rows) });
});
...
app.post("/api/projects/:projectId/bim-elements", async (req, res) => {
  ...
  const { rows } = await pool.query(
    ...
    [projectId, body.externalId, body.elementType, body.name, body.levelName,
...
app.post("/api/bim/link-test", async (req, res) => {
  ...
  const { rows } = await pool.query(
    `update material_tests set bim_element_id=$3
     where project_id=$1 and id=$2
     and exists(select 1 from bim_elements where project_id=$1 and id=$3)
     returning *`,
    [body.projectId, body.testId, body.bimElementId],
  );
  if (!rows[0]) return res.status(404).json({ error: "Project-scoped test or BIM element not found." });
...
app.post("/api/bim/fabrication-plan", async (req, res) => {
  ...
  const { rows } = await pool.query(
    `insert into fabrication_plans(
      ...
      returning *`,
    [body.projectId, body.bimElementId, body.moduleName, body.processType, body.estimatedTimeHours,
      body.wastePercent, JSON.stringify(body.qaCheckpoints), JSON.stringify(body.risks)],
  );
  if (!rows[0]) return res.status(404).json({ error: "Project-scoped BIM element not found." });
...
app.get("/api/projects/:projectId/reports", async (req, res) => {
  const projectId = z.string().uuid().parse(req.params.projectId);
  const { rows } = await pool.query(
    "select * from ai_reports where project_id=$1 order by created_at desc",
    [projectId],
  );
  res.json(toCamelCase(rows));
});
...
app.get("/api/projects/:projectId/fabrication-plans", async (req, res) => {
  const projectId = z.string().uuid().parse(req.params.projectId);
  const { rows } = await pool.query(
    "select * from fabrication_plans where project_id=$1 order by created_at desc",
    [projectId],
  );
  res.json(toCamelCase(rows));
});
...
app.post("/api/projects/:projectId/fabrication-plans", async (req, res) => {
  ...
  const { rows } = await pool.query(
    ...
    [projectId, body.bimElementId, body.moduleName, body.processType, body.estimatedTimeHours,
      body.wastePercent, JSON.stringify(body.qaCheckpoints), JSON.stringify(body.risks)],
  );
  res.status(201).json(toCamelCase(rows[0]));
});
...
app.get(["/api/model-registry", "/api/models"], async (req, res) => {
  ...
  const { rows } = await pool.query(
    `select * from model_registry ${where} order by trained_at desc`,
    values,
  );
  res.json(toCamelCase(rows));
});
...
app.get(["/api/model-registry/:id", "/api/models/:id"], async (req, res) => {
  ...
  if (!rows[0]) return res.status(404).json({ error: "Model registry entry not found." });
  res.json(toCamelCase(rows[0]));
});
...
app.post(["/api/model-registry", "/api/models"], async (req, res) => {
  ...
  try {
    ...
    const { rows } = await client.query(
      `insert into model_registry(model_name,version,task,feature_set,metrics,artifact_path,trained_at,active,notes)
       values($1,$2,$3,$4,$5,$6,coalesce($7,now()),$8,$9) returning *`,
      [body.modelName, body.version, body.task, JSON.stringify(body.featureSet),
...
app.patch(["/api/model-registry/:id", "/api/models/:id"], async (req, res) => {
  ...
  try {
    ...
    const { rows } = await client.query(
      `update model_registry set model_name=$2,version=$3,task=$4,feature_set=$5,metrics=$6,
       artifact_path=$7,trained_at=$8,active=$9,notes=$10 where id=$1 returning *`,
      [id, updates.modelName ?? existing.model_name, updates.version ?? existing.version, task,
...
app.post(["/api/model-registry/:id/activate", "/api/models/:id/activate"], async (req, res) => {
  ...
  try {
    ...
    const { rows } = await client.query(
      "update model_registry set active=true where id=$1 returning *",
      [id],
    );
    await client.query("commit");
    res.json(toCamelCase(rows[0]));
...
app.delete(["/api/model-registry/:id", "/api/models/:id"], async (req, res) => {
  ...
  if (!rowCount) return res.status(404).json({ error: "Model registry entry not found." });
  res.status(204).send();
});
...
app.post("/api/materials", async (req, res) => {
  ...
  const { rows } = await pool.query(
    `insert into materials(project_id,name,category,spec)
     values($1,$2,$3,$4) returning id, project_id as "projectId", name, category, spec`,
    [body.projectId, body.name, body.category, body.spec],
  );
...
app.post("/api/projects/:projectId/bim-link", async (req, res) => {
  ...
  try {
    ...
    const elements = body.bimElementIds.length
      ? await client.query(
        "update bim_elements set material_id=$1 where project_id=$2 and id=any($3::uuid[])",
        [body.materialId, projectId, body.bimElementIds],
      )
      : { rowCount: 0 };
    const tests = body.testIds.length
      ? await client.query(
        `update material_tests set material_id=$1,
         bim_element_id=coalesce(bim_element_id,$4::uuid)
         where project_id=$2 and id=any($3::uuid[])`,
        [body.materialId, projectId, body.testIds, body.bimElementIds[0] ?? null],
      )
      : { rowCount: 0 };
    await client.query("commit");
    res.json({
...
const proxyJSON = async (url: string, body: unknown) => {
...
app.post(["/ai/materials/predict", "/api/materials/predict"], async (req, res) => {
  ...
  const models = result.models && typeof result.models === "object"
    ? Object.values(result.models as Record<string, unknown>).map(String)
    : [];
  res.json({
...
app.post("/ai/qc-report", async (req, res) => {
  ...
  const { rows } = await pool.query(
    "select * from material_tests where project_id=$1 and id = any($2::uuid[])",
    [body.projectId, body.testIds],
  );
...
app.post("/api/reports/material-test", async (req, res) => {
  ...
  const { rows } = await pool.query(
    `select material_tests.*, materials.name as material_name, bim_elements.external_guid
     from material_tests
     ...
     and (cardinality($2::uuid[])=0 or material_tests.id=any($2::uuid[]))`,
    [body.projectId, body.testIds],
  );
  const markdown = await llm.generateQCReport(rows);
  await pool.query(
    `insert into ai_reports(project_id,report_type,title,content_md,source_payload)
     values($1,'material_test',$2,$3,$4)`,
    [body.projectId, "Material Test Report", markdown, { testIds: body.testIds }],
  );
  res.json({ markdown });
});
...
app.post("/ai/design-copilot", async (req, res) => {
  res.json(await llm.structured(
    "Suggest 3 BIM-linked envelope and structure strategies with energy, carbon, daylight, and acoustic estimates. Return JSON.",
    req.body,
  ));
});
...
app.post("/ai/fabrication-planner", async (req, res) => {
  res.json(await llm.structured(
    "Partition BIM assemblies into manufacturable modules and return modules, process_plan, QA checkpoints, waste, time, and risks as JSON.",
    req.body,
  ));
});
